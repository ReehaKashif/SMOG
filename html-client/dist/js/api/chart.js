...
const plotSmogCausesChart = (data) => {
  ...
  const darkRedStartDate = new Date();
  darkRedStartDate.setDate(darkRedStartDate.getDate() + 1); // Start tomorrow

  const last7DaysData = [];
  for (let i = 7; i > 0; i--) {
    const date = new Date(darkRedStartDate);
    date.setDate(darkRedStartDate.getDate() - i); // Move back 7 days
    const formattedDate = convertDateFormat(date, false);

    const pastIndex = this_year_past_two_months.date.findIndex(
      (d) => convertDateFormat(d, false) === formattedDate
    );
    const aqiValue = pastIndex !== -1 ? this_year_past_two_months.aqi[pastIndex] + 10 : null; // Add 10 to AQI value
    last7DaysData.push(aqiValue);
  }
