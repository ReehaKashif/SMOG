...
const plotSmogCausesChart = (data) => {
    console.log("Data received in plotSmogCausesChart:", data);
    ...
    // Log raw data for each dataset
    console.log("Last Year Next Two Months Data:", lastYearNextTwoMonths);
    console.log("Last Year Past Two Months Data:", lastYearPastTwoMonths);
    console.log("This Year Next Two Months Data:", thisYearNextTwoMonths);
    console.log("This Year Past Two Months Data:", thisYearPastTwoMonths);
    console.log("Last Month Data (Raw):", lastMonthData);
    ...
    console.log("Generated Date Range:", dateRange.map(date => date.toISOString().split('T')[0]));

    // Map data to the date range
    const mapDataToDateRange = (dataSet) => {
        console.log("Mapping Data Set:", dataSet);
        return dateRange.map((date) => {
            ...
            const value = index !== -1 ? dataSet.aqi[index] : null;
            if (index !== -1) {
                console.log(`Matched Date: ${formattedDate}, AQI Value: ${value}`);
            }
            return value;
        });
    };
    ...
    // Directly map the last month data using the "Date" and "AQI" fields
    const lastMonthMappedData = dateRange.map((date) => {
        const formattedDate = date.toLocaleDateString("en-US"); // Matches the "MM/DD/YYYY" format
        const index = lastMonthData.Date.indexOf(formattedDate);
        if (index !== -1) {
            console.log(`Last Month Matched Date: ${formattedDate}, AQI Value: ${lastMonthData.AQI[index]}`);
        }
        return index !== -1 ? lastMonthData.AQI[index] : null;
    });

    console.log("Final Last Month Mapped Data:", lastMonthMappedData);
    ...
    const datasets = [
            ...
            segment: {
                borderColor: function(context) {
                    ...
                    if (diffInDays <= 7 && diffInDays > 0) {
                        ...
                        return '#ff0000'; // Default Red for past dates
                    }
                }
            },
            fill: false,
        ...
        {
            label: "Current Year (Last Month)",
            data: lastMonthMappedData, // Use mapped last month data directly
