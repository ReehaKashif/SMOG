...
const plotSmogCausesChart = (data) => {
    ...
    // Create an array for the green line data
    const greenLineData = dateRange.map((date) => {
        const formattedDate = convertDateFormat(date, false);
        const index = dateRange.findIndex(d => convertDateFormat(d, false) === formattedDate);

        // Logging for debugging
        console.log("Formatted Date:", formattedDate);
        
        // Set the y-value 1 or 2 points above the current year's AQI
        if (formattedDate >= '2024-10-25' && formattedDate <= '2024-10-31') {
            const currentAqi = thisYearData[index];
            console.log("Current AQI for", formattedDate, ":", currentAqi);
            return (currentAqi !== null) ? currentAqi + 2 : null; // Adjust as needed (1 or 2 points above)
        }
        return null; // No value outside the specified range
    });

    // Optional: Test with fixed data for the green line (uncomment for testing)
    // const greenLineData = dateRange.map((date) => {
    //     const formattedDate = convertDateFormat(date, false);
    //     if (formattedDate >= '2024-10-25' && formattedDate <= '2024-11-01') {
    //         return 150; // Set a fixed y-value for testing
    //     }
    //     return null;
    // });
    ...
    const datasets = [
        ...
        // Green line dataset
        {
            ...
            fill: false,
            tension: 0.1,
            pointStyle: false, // No points on the line
        },
    ];
