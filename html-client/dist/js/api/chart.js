...
const plotSmogCausesChart = (data) => {
  ...
  const last7DaysData = [];
  const currentDateString = this_year_next_two_months.date[0]; // Current date
  const currentAQI = this_year_next_two_months.aqi[0] + 10; // AQI value for today
  last7DaysData.push(currentAQI);

  // Collect last 6 days data
  for (let i = 1; i <= 6; i++) {
      const pastDate = new Date();
      pastDate.setDate(pastDate.getDate() - i); // Go back one day
  ...
  // Create an array to hold AQI data aligned with the dateRange
  ...
  const datasets = [
      ...
      {
          label: "Last 7 days",
          data: last7DaysCombinedData, // This now should align properly
