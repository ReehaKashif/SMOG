...
const plotSmogCausesChart = (data) => {
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      dateRange.push(new Date(d).toISOString().split('T')[0]); // Convert to "YYYY-MM-DD" format
  }
  ...
  // Get the last 7 days data with current date from next two months and last 6 days from past two months
  const last7DaysData = [];
  const last7DaysLabels = [];

  // Current date from the next two months
  const currentFormattedDate = this_year_next_two_months.date[0]; // Get the current date from next two months
  last7DaysData.push(this_year_next_two_months.aqi[0] + 10); // Add 10 to the AQI value
  ...
  // Combine all date labels
  const allLabels = dateRange;
