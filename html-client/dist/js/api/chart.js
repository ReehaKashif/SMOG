...
const plotSmogCausesChart = (data) => {
    ...
    // Normalize dates from the API response
    const last_month_dates = data["lastMonthData"]?.["Date"].map(date => {
        const [month, day, year] = date.split("/");
        return `${year}-${month.padStart(2, "0")}-${day.padStart(2, "0")}`; // Convert to YYYY-MM-DD
    }) || [];

    const last_month_aqi = data["lastMonthData"]?.["AQI"] || [];

    console.log("Normalized Last Month Dates:", last_month_dates);

    const currentDate = new Date();
    const startDate = new Date(Math.min(...last_month_dates.map(date => new Date(date))));
    const endDate = new Date(Math.max(...last_month_dates.map(date => new Date(date))));
    ...
    console.log("Date Range (formatted):", dateRange.map(date => date.toISOString().split('T')[0]));

    const lastMonthData = dateRange.map((date) => {
        const formattedDate = date.toISOString().split('T')[0]; // Format date as YYYY-MM-DD
        const index = last_month_dates.indexOf(formattedDate); // Find exact match
        console.log(`Checking date: ${formattedDate}, Found Index: ${index}, AQI: ${index !== -1 ? last_month_aqi[index] : null}`);
        return index !== -1 ? last_month_aqi[index] : null;
    });
