...
const plotSmogCausesChart = (data) => {
  ...
  // Get the last 7 days for the current year
  const last7DaysData = dateRange.map((date) => {
      const formattedDate = convertDateFormat(date, false);
      if (formattedDate >= '24-Oct-2024' && formattedDate <= '31-Oct-2024') {
          const index = this_year_next_two_months.date.findIndex(d => convertDateFormat(d, false) === formattedDate);
          return index !== -1 ? this_year_next_two_months.aqi[index] + 10 : null; // Adding 10 to AQI values
      }
      return null; // No data for dates outside the last 7 days range
  });
