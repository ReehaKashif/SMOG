...
const plotSmogCausesChart = (data) => {
  ...
  // Adjusted data 7 days back from the red line by ±3–4 units
  const sevenDaysBackData = roundNullableData(
    padListWithNull(
      this_year_next_two_months.aqi.slice(0, 7).map(value => value + (Math.random() < 0.5 ? -3 : 4)),
      last_year_past_two_months.date.length +
        last_year_next_two_months.date.length +
        this_year_past_two_months.date.length - 7
    )
  );

  const datasets = [
    // Previous Year, Current Year, Next 7 days, Next 14 days, After 14 days datasets (already defined)

    // New dataset for 7 days back with adjusted values
    {
      label: "7 Days Back (±3–4 Adjusted)",
      data: sevenDaysBackData,
      backgroundColor: "#FFA500", // Orange color for adjusted line
      borderColor: "#FFA500", // Orange color for adjusted line
      fill: false,
      tension: 0.1
    }
  ];
