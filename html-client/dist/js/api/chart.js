...
const plotSmogCausesChart = (data) => { 
    ...
    // Adjust the last 7 days line to connect with next 7 days line
    const last7DaysData = this_year_past_two_months.aqi.slice(-7); // Get the last 7 days data
    const next7DaysData = this_year_next_two_months.aqi.slice(0, 7); // Get the next 7 days data
  
    // Combine last 7 days and next 7 days data for seamless connection
    const combinedLast7AndNext7DaysData = last7DaysData.concat(next7DaysData);
    ...
    const datasets = [
      ...
      },
      {
        label: "Next 7 Days",
        data: next7DaysData,
        backgroundColor: "#8B0000", // Dark red for next 7 days
        borderColor: "#8B0000", // Dark red for next 7 days
        fill: false,
        tension: 0.1
      },
      {
