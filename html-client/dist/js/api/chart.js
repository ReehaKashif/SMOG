...
const plotSmogCausesChart = (data) => { 
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      dateRange.push(new Date(d)); // Add days in the range
    }
    ...
    // Define Last 7 Days explicitly from October 25 to November 1
    const last7Days = [
      '25-Oct-2024', '26-Oct-2024', '27-Oct-2024', 
      '28-Oct-2024', '29-Oct-2024', '30-Oct-2024', 
      '31-Oct-2024', '01-Nov-2024'
    ];
  
    const last7DaysAQI = last7Days.map(date => {
      const index = this_year_past_two_months.date.findIndex(d => convertDateFormat(d, false) === date);
      return index !== -1 ? this_year_past_two_months.aqi[index] : null; // No data for that date
    });
    ...
    const datasets = [
      ...
      {
        label: "Last 7 Days",
        data: last7DaysAQI, // Use the last 7 days data calculated
