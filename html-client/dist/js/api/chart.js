...
const plotSmogCausesChart = (data) => {
  ...
  // Current date from next two months
  const currentFormattedDate = this_year_next_two_months.date[0]; // Get the current date from next two months
  const currentAQI = this_year_next_two_months.aqi[0] + 10; // Add 10 to the AQI value
  last7DaysData.push(currentAQI);
  last7DaysLabels.push(currentFormattedDate);

  // Last 6 days from past two months
  for (let i = 1; i <= 6; i++) {
      ...
      // Collect the data for last 6 days, adding 10 to the AQI values
      last7DaysData.unshift(indexPast !== -1 ? this_year_past_two_months.aqi[indexPast] + 10 : null);
      last7DaysLabels.unshift(formattedPastDate);
  }

  // Combine last 7 days labels with the date range
  const allLabels = dateRange; // Use the full date range for x-axis labels
  const last7DaysCombinedData = last7DaysData.concat(new Array(dateRange.length - last7DaysData.length).fill(null));
  ...
  const datasets = [
      ...
      {
          label: "Last 7 days",
          data: last7DaysCombinedData, // Align last 7 days with the appropriate date labels
  ...
  predictionChart = new Chart(predictionChartCtx, {
      type: "line",
      data: {
          labels: allLabels, // Ensure labels correspond to the correct date range
          datasets: datasets,
      },
      options: {
          responsive: true,
