...
const plotSmogCausesChart = (data) => { 
    ...
    // Prepare data for the green line, starting from the Current Year data plus 1 value
    const greenLineData = dateRange.map((date, index) => {
      if (index >= startGreenIndex && index <= endGreenIndex) {
        return (thisYearData[index] !== null) ? thisYearData[index] + 1 : null; // Add 1 to the Current Year data
      }
