...
const plotSmogCausesChart = (data) => {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const indexCurrent = data["this_year_data"]["next_two_months"].date.indexOf(formattedPastDate);
        const indexPast = data["this_year_data"]["past_two_months"].date.indexOf(formattedPastDate);

        // Collect the data for the last 7 days, adding 10 to the AQI values
        const aqiValue = (indexCurrent !== -1 ? data["this_year_data"]["next_two_months"].aqi[indexCurrent] :
            (indexPast !== -1 ? data["this_year_data"]["past_two_months"].aqi[indexPast] : null));

        // Check if aqiValue is not null, then add 10
        last7DaysData.unshift(aqiValue !== null ? aqiValue + 10 : null);
        last7DaysLabels.unshift(formattedPastDate);
    }

    // Prepare last 7 days data aligned with dateRange (only for the last 7 days in dateRange)
    const last7DaysCombinedData = dateRange.slice(-7).map((date) => {
        const formattedDate = convertDateFormat(date, false);
        const last7DaysIndex = last7DaysLabels.indexOf(formattedDate);
    ...
    const datasets = [
        ...
        },
        {
            ...
            backgroundColor: "#008000", // Green for last 7 days (with +10 added)
