...
const plotSmogCausesChart = (data) => {
    ...
    // Create adjusted line data from the last 7 days of the black line
    const adjustedLineData = dateRange.map((date, index) => {
      const last7DaysIndex = thisYearData.length - 7 + index; // Get index for last 7 days
      if (last7DaysIndex >= thisYearData.length) return null; // Out of bounds for the last 7 days
  
      const currentYearValue = thisYearData[last7DaysIndex]; // Get current year's value (black line)
  
      // If the current year's value exists, adjust it
      if (currentYearValue !== null) {
        const adjustment = (index % 2 === 0) ? 3 : -4; // Alternating +3 and -4
        return currentYearValue + adjustment;
      }
      return null; // Return null if there's no current year value
    }).slice(0, 7); // Ensure it has only 7 values
