...
const plotSmogCausesChart = (data) => {
    ...
    // Dynamic AQI range for the chart
    const allDataValues = [
        ...lastYearData.filter(value => value !== null),
        ...thisYearData.filter(value => value !== null),
        ...lastMonthCombinedData.filter(value => value !== null)
    ];
    const minAQI = Math.min(...allDataValues);
    const maxAQI = Math.max(...allDataValues);
    ...
    predictionChart = new Chart(predictionChartCtx, {
        ...
        options: {
            ...
            scales: {
                y: {
                    beginAtZero: false,
                    min: Math.max(0, minAQI - 50), // Dynamic minimum
                    max: Math.min(800, maxAQI + 50), // Dynamic maximum
                },
                x: {
                    type: "category",
                    title: {
