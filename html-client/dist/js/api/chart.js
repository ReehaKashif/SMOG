...
const plotSmogCausesChart = (data) => {
  ...
  const convertDateFormat = (date, includeYear = true) => {
  ...
  // Get the last 7 days data from current date to the past with +10 AQI value adjustment
  const last7DaysData = [];
  ...
  for (let i = 0; i < 7; i++) {
      ...
      // Collect the data for last 7 days, adding 10 to the AQI values
      last7DaysData.unshift(indexPast !== -1 ? this_year_past_two_months.aqi[indexPast] + 10 : null);
      last7DaysLabels.unshift(formattedDate);
  }
