...
const plotSmogCausesChart = (data) => {
    ...
    const last7Days = thisYearData.slice(-7).reverse(); // Get the last 7 days in reverse order
    const adjustedLineData = last7Days.map((value, index) => {
        if (value !== null) {
            const adjustment = (index % 2 === 0) ? -3 : 2; // Alternating -3 and +2 to run backward
