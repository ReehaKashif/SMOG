...
const plotSmogCausesChart = (data) => {
  ...
  // Generate the date range from two months ago to December 26, 2024
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      dateRange.push(d.toISOString().split('T')[0]); // Convert to "YYYY-MM-DD" format
  }

  // Map AQI data for last year
  const lastYearData = dateRange.map((date) => {
      const pastIndex = last_year_past_two_months.date.indexOf(date);
      const nextIndex = last_year_next_two_months.date.indexOf(date);
  ...
  const thisYearData = dateRange.map((date) => {
      const pastIndex = this_year_past_two_months.date.indexOf(date);
      const nextIndex = this_year_next_two_months.date.indexOf(date);
  ...
  // Collect the last 7 days of AQI data
  for (let i = 0; i < 7; i++) {
      ...
      const indexCurrent = this_year_next_two_months.date.indexOf(formattedPastDate);
      const indexPast = this_year_past_two_months.date.indexOf(formattedPastDate);

      // Collect the data for the last 7 days, adding 10 to the AQI values
      const aqiValue = (indexCurrent !== -1 ? this_year_next_two_months.aqi[indexCurrent] : 
                       (indexPast !== -1 ? this_year_past_two_months.aqi[indexPast] : null)) + 10;
      last7DaysData.unshift(aqiValue);
      last7DaysLabels.unshift(formattedPastDate);
  }

  // Prepare last 7 days data aligned with dateRange
  const last7DaysCombinedData = dateRange.map((date) => {
      const last7DaysIndex = last7DaysLabels.indexOf(date);
      return last7DaysIndex !== -1 ? last7DaysData[last7DaysIndex] : null;
  });
  ...
  const datasets = [
      ...
      },
      {
          ...
          backgroundColor: "#008000", // Green color for last 7 days
          borderColor: "#008000", // Green color for last 7 days
          fill: false,
          tension: 0.1,
      },
      {
          label: "Last 7 days connection",
          data: last7DaysData,
          backgroundColor: "#008000", // Use same green color for connection line
          borderColor: "#008000", // Use same green color for connection line
          fill: false,
          tension: 0.1,
  ...
  predictionChart = new Chart(predictionChartCtx, {
      type: "line",
      data: {
          labels: dateRange, // Ensure labels correspond to the correct date range
          datasets: datasets,
      },
      options: {
          responsive: true,
