...
const plotSmogCausesChart = (data) => {
    ...
    // Validate lastMonthData structure
    if (!data || !data["lastMonthData"]) {
        console.error("Missing lastMonthData in the provided data:", data);
        return;
    }

    if (!data["lastMonthData"]["Date"] || !data["lastMonthData"]["AQI"]) {
        console.error("Invalid lastMonthData structure:", data["lastMonthData"]);
        return;
    }
    ...
    // Normalize Last Month Dates
    const lastMonthDates = data["lastMonthData"]["Date"].map(date => {
        if (!date.includes("/")) {
    ...
    const lastMonthAQI = data["lastMonthData"]["AQI"];
