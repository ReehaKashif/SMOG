...
const plotSmogCausesChart = (data) => {
  ...
  // Get the last 7 days data from past two months
  const last7DaysData = [];
  const last7DaysDates = [];
  for (let i = 0; i < 7; i++) {
      const date = new Date();
      date.setDate(date.getDate() - i); // Go back one day at a time
      const formattedDate = convertDateFormat(date, false);
      
      // Find the index for the past two months' data
      const indexPast = this_year_past_two_months.date.findIndex(d => convertDateFormat(d, false) === formattedDate);
      
      // Collect the data for last 7 days
      last7DaysData.unshift(indexPast !== -1 ? this_year_past_two_months.aqi[indexPast] : null);
      last7DaysDates.unshift(formattedDate); // Collect corresponding formatted dates
  }

  // Create a complete data set for the x-axis, including last 7 days
  const allLabels = dateRange.map((date) => convertDateFormat(date, false));
  const last7DaysLabelMapping = last7DaysDates.map(date => allLabels.includes(date) ? date : null);
  ...
  predictionChart = new Chart(predictionChartCtx, {
      type: "line",
      data: {
          labels: allLabels, // Use the generated dateRange for the x-axis
