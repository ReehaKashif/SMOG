...
const plotSmogCausesChart = (data) => {
  ...
  // Prepare last month data from the current year's past two months
  const lastMonthCombinedData = new Array(dateRange.length).fill(null);
  const lastMonthDays = 30; // Change to 30 days for the last month
  ...
  for (let i = 0; i < lastMonthDays; i++) {
      ...
      if (dateIndex !== -1) {
          lastMonthCombinedData[dateIndex] = adjustedAqiValue;
      }
  ...
  const datasets = [
      ...
      {
          label: "Last Month", // Changed label here
          data: lastMonthCombinedData,
          backgroundColor: "#ff0000", // Set to red
          borderColor: "#ff0000", // Set to red
          fill: false,
          tension: 0.1,
      },
  ];
