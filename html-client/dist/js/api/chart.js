...
const plotSmogCausesChart = (data) => {
  ...
  // Calculate the last 7 days from the start of the "dark red" period in the current year data, with AQI values +10
  const darkRedStartIndex = dateRange.findIndex((date) => {
    const formattedDate = convertDateFormat(date, false);
    return this_year_next_two_months.date.some(
      (d, i) => i < 7 && convertDateFormat(d, false) === formattedDate
    );
  });

  const last7DaysData = Array(dateRange.length).fill(null); // Initialize with null values
  if (darkRedStartIndex > 7) {
    for (let i = 0; i < 7; i++) {
      const index = darkRedStartIndex - 7 + i;
      last7DaysData[index] = this_year_past_two_months.aqi[index] + 10; // AQI values +10
    }
  }
  ...
  const datasets = [
    ...
    },
    {
      ...
      pointStyle: false,
      tension: 0.1,
      spanGaps: true, // Connect gaps in the current year data
    },
    {
      label: "Last 7 Days",
      data: last7DaysData,
      ...
      fill: false,
      tension: 0.1,
      spanGaps: true, // Connect gaps in the last 7 days data
    },
    {
      label: "Next 7 days",
      ...
      fill: false,
      tension: 0.1,
      spanGaps: true, // Connect gaps in the next 7 days data
    },
    {
      label: "Next 14 days",
      data: roundNullableData(
