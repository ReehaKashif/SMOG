...
const plotSmogCausesChart = (data) => { 
    ...
    const endDate = new Date('14-Dec-2024'); // Fixed end date
    ...
    // Get index for the date range for the green line
    const startGreenIndex = dateRange.findIndex(date => convertDateFormat(date, false) === '25-Oct-2024');
    const endGreenIndex = dateRange.findIndex(date => convertDateFormat(date, false) === '01-Nov-2024');
  
    // Prepare data for the green line, starting from the Current Year data plus 2 values
    const greenLineData = dateRange.map((date, index) => {
      if (index >= startGreenIndex && index <= endGreenIndex) {
        return (thisYearData[index] !== null) ? thisYearData[index] + 2 : null; // Add 2 to the Current Year data
      }
      return null; // No data outside the specified range
    });
    ...
    const datasets = [
      ...
      },
      {
        ...
        backgroundColor: "#FF7F7F", // Light red for beyond 14 days
        borderColor: "#FF7F7F", // Light red for beyond 14 days
        fill: false,
        tension: 0.1
      },
      // New green line dataset
      {
        label: "Green Line",
        data: greenLineData,
