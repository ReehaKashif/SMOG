...
const plotSmogCausesChart = (data) => {
    ...
    // Create adjusted line data starting from the Dark Red Line for the last 7 days
    const future7DaysData = this_year_next_two_months.aqi.slice(0, 7); // Dark Red Line AQI values
    const last7DaysBlackLineData = thisYearData.slice(-7); // Last 7 days from the Current Year AQI

    // Adjusting last 7 days backward from the Dark Red Line
    const adjustedLineData = last7DaysBlackLineData.map((blackLineValue, index) => {
        if (blackLineValue !== null) {
            const adjustment = (index % 2 === 0) ? -3 : 2; // Alternating -3 and +2
            return blackLineValue + adjustment;
        }
        return null; // Return null if there's no current year value
    }).reverse(); // Reverse to match backward adjustment

    // Merge adjustedLineData with the first point of the dark red line
    adjustedLineData.unshift(this_year_next_two_months.aqi[0]); // Add the dark red line's starting point

    // Define the green line data (example: last 10 to 3 days from current year AQI)
    const greenLineData = thisYearData.slice(-10, -3);
    ...
    const datasets = [
        ...
        },
        {
            ...
            backgroundColor: "#FF7F7F", // Light red for after 14 days
            borderColor: "#FF7F7F", // Light red for after 14 days
            fill: false,
            tension: 0.1
        },
        {
            ...
            backgroundColor: "#FF4500", // Orange color for the connected line
            borderColor: "#FF4500", // Orange color for the connected line
            fill: false,
            tension: 0.1,
            pointStyle: false,
            spanGaps: true,
        },
        {
            label: "Green Line",
            data: greenLineData,
            backgroundColor: "#008000", // Green color for the line
            borderColor: "#008000", // Green color for the line
            fill: false,
            tension: 0.1,
            pointStyle: false,
            spanGaps: true,
