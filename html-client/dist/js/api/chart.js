...
const plotSmogCausesChart = (data) => {
    ...
    // Prepare the Last 7 Days data (Assuming the current date is '31-Oct-2024')
    const last7Days = [];
    const last7DaysAQI = [];
  
    for (let i = 0; i < 7; i++) {
      const date = new Date();
      date.setDate(currentDate.getDate() - i); // Get the last 7 days
      const formattedDate = convertDateFormat(date, false);
      last7Days.push(formattedDate);
  
      const index = this_year_past_two_months.date.findIndex(d => convertDateFormat(d, false) === formattedDate);
      last7DaysAQI.push(index !== -1 ? this_year_past_two_months.aqi[index] : null);
    }
