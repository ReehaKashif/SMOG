...
const plotSmogCausesChart = (data) => {
    ...
    const lastMonthCombinedData = new Array(dateRange.length).fill(null);
    ...
    for (let i = 0; i < lastMonthDays; i++) {
        ...
        if (dateIndex !== -1) {
            lastMonthCombinedData[dateIndex] = aqiValue;
        }
        console.log(`For date ${formattedPastDate}, AQI: ${aqiValue}, Date Index in dateRange: ${dateIndex}`);
    }
  
    console.log("Final Last Month Data:", lastMonthCombinedData);
            ...
            segment: {
                borderColor: function(context) {
                    ...
                    if (diffInDays <= 7 && diffInDays > 0) {
                        ...
                        return '#000000'; // Default Black for past dates
                    }
                }
            },
            fill: false,
...
async function fetchAQIPredictionData() {
  ...
  async function fetchMaxAQIData() {
      try {
          ...
          const currentDate = moment().utcOffset(5).format("MM/DD/YYYY");
          const currentHour = moment().utcOffset(5).hour();

          const response = await fetch(`https://smog-aqi-server.onrender.com/api/hourly_aqi/${district}?date=${currentDate}`);
          ...
          const maxAqiValues = [];
          for (let i = 0; i < 24; i++) {
              const hourIndex = (currentHour - i + 24) % 24;  // Handle wrapping of hours from 0-23
              const hour = data.data.hours.find(h => parseInt(h) === hourIndex);

              if (hour) {
                  hours.unshift(hour);  // Add hour to the beginning
                  const maxAqi = Math.max(...data.data.aqi.slice(hourIndex * 3, hourIndex * 3 + 3));  // Maximum AQI for that hour
                  maxAqiValues.unshift(maxAqi);  // Add AQI to the beginning
              }
          }
