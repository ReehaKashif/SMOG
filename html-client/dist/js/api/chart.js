...
const plotSmogCausesChart = (data) => {
  ...
  // Get the last 7 days data for the current year
  const last7DaysData = [];
  for (let i = 0; i < 7; i++) {
      const date = new Date();
      ...
      // Find the index for the current year's data
      const index = this_year_next_two_months.date.findIndex(d => convertDateFormat(d, false) === formattedDate);
      last7DaysData.unshift(index !== -1 ? this_year_next_two_months.aqi[index] + 10 : null);
  }
