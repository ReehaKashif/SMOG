...
const plotSmogCausesChart = (data) => {
    console.log("Data received for the chart:", data);

    // Validate that Date and AQI keys exist in the response
    if (!data || !data["Date"] || !data["AQI"]) {
        console.error("Missing Date or AQI in the provided data:", data);
        return;
    }

    // Normalize Date field to YYYY-MM-DD format
    const lastMonthDates = data["Date"].map(date => {
        if (!date.includes("/")) {
    ...
    }).filter(Boolean); // Remove null or invalid dates

    const lastMonthAQI = data["AQI"];

    const currentDate = new Date();

    // Set start and end dates for the previous two months
    const startDate = new Date(currentDate);
    startDate.setMonth(currentDate.getMonth() - 2); // Two months before current date
    const endDate = new Date(currentDate);
    endDate.setDate(currentDate.getDate() - 1); // Up to yesterday
    ...
    console.log("Date range for the chart (Last 2 months):", dateRange.map(date => date.toISOString().split('T')[0]));

    // Map AQI values for the Date range
    const lastMonthData = dateRange.map((date) => {
        const formattedDate = date.toISOString().split('T')[0];
        const index = lastMonthDates.indexOf(formattedDate); // Match with normalized lastMonthDates
        return index !== -1 ? lastMonthAQI[index] : null;
    });

    console.log("Final Last Month Data:", lastMonthData);

    // Other datasets
    const lastYearData = new Array(dateRange.length).fill(150); // Dummy example for previous year data
    const thisYearData = new Array(dateRange.length).fill(200); // Dummy example for prediction data
    const currentYearData = new Array(dateRange.length).fill(180); // Dummy example for current year data
    ...
    const datasets = [
        ...
        {
            label: "Last Month (Last 2 Months)",
            data: lastMonthData,
    ...
    predictionChart = new Chart(predictionChartCtx, {
        type: "line",
        data: {
            labels: dateRange.map(date => date.toISOString().split('T')[0]),
            datasets: datasets,
        },
        options: {
            responsive: true,
...
async function fetchAQIPredictionData() {
  try {
      ...
      // Process data
      const datesThisYear = thisYearData.before_period.date.concat(thisYearData.after_period.date);
      const minThisYear = thisYearData.before_period.min.concat(thisYearData.after_period.min);
      const maxThisYear = thisYearData.before_period.max.concat(thisYearData.after_period.max);
      const datesLastYear = lastYearData.before_period.date.concat(lastYearData.after_period.date);
      const minLastYear = lastYearData.before_period.min.concat(lastYearData.after_period.min);
      ...
      aqiPredictionChart = new Chart(aqiPredictionChartctx, {
          type: 'line',
          data: {
              labels: datesThisYear,
              datasets: [
                  {
                      label: 'Min AQI This Year',
                      data: minThisYear,
