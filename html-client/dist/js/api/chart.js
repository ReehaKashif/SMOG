...
const plotSmogCausesChart = (data) => {
  ...
  // Get the last 7 days data from current date to the past
  const last7DaysData = [];
  ...
  for (let i = 0; i < 7; i++) {
      ...
      // Find the index for the current year's data in past two months
      ...
      last7DaysLabels.unshift(formattedDate); // Collect corresponding formatted dates
  }

  // Combine all date labels, ensuring the last 7 days are included correctly
