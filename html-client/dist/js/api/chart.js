...
// Calculate the start date of the dark red line
const darkRedStartDate = new Date(this_year_next_two_months.date[0]);

// Calculate the start and end dates for the green line (7 days before the dark red start date)
const greenLineEndDate = new Date(darkRedStartDate);
const greenLineStartDate = new Date(darkRedStartDate);
greenLineStartDate.setDate(greenLineStartDate.getDate() - 6); // 7 days in total

// Filter thisYearData for the green line based on the calculated date range
const greenLineData = [];
const greenLineDates = [];

for (let i = dateRange.length - 1; i >= 0; i--) {
    const date = dateRange[i];
    if (date >= greenLineStartDate && date <= greenLineEndDate) {
        greenLineData.push(thisYearData[i]);
        greenLineDates.push(convertDateFormat(date, false));
    }
}

// Adding the green line dataset to the chart
const datasets = [
    {
        label: "Previous Year",
        data: lastYearData,
    ...
    },
    {
        ...
        data: roundNullableData(
            padListWithNull(
                greenLineData.reverse(), // Reversed to show it in correct order
                last_year_past_two_months.date.length +
                last_year_next_two_months.date.length +
                this_year_past_two_months.date.length -
                greenLineData.length
            )
        ),
        backgroundColor: "#008000", // Green color for the last 7 days
        borderColor: "#008000", // Green color for the last 7 days
        fill: false,
        tension: 0.1,
        pointStyle: false,
        spanGaps: true,
...
// Configure the chart with the updated datasets
if (predictionChart) {
