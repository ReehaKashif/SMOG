...
const plotSmogCausesChart = async () => {
    // Fetch data from the API
    const response = await fetch("https://smog-server.onrender.com/api/this_year?district=Lahore");
    ...
    // Prepare the Last 7 Days data
    const last7Days = [];
    const last7DaysAQI = [];
  
    const today = new Date();
    const last7DaysResponse = await fetch("https://smog-server.onrender.com/api/last_7_days?district=Lahore");
    const last7DaysData = await last7DaysResponse.json(); // Assuming the API returns the last 7 days' data
  
    for (let i = 0; i < 7; i++) {
      const date = new Date();
      date.setDate(today.getDate() - i); // Get the last 7 days
      const formattedDate = convertDateFormat(date, false);
      last7Days.push(formattedDate);
  
      const index = last7DaysData.date.findIndex(d => convertDateFormat(d, false) === formattedDate);
      last7DaysAQI.push(index !== -1 ? last7DaysData.aqi[index] : null);
    }
  
    // Reverse the last7Days and last7DaysAQI to start from the oldest to the newest
    last7Days.reverse();
    last7DaysAQI.reverse();
