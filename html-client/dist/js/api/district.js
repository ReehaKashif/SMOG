...
const processAndCacheData = (data) => {
  ...
  districts.forEach((district) => {
    ...
    $("#greenery-district-selector").append(`<option>${district}</option>`);
  });
...
function getAQIColor(aqi) {
  if (aqi <= 50) {
      ...
      return '#ff6501';  // Orange
  } else if (aqi <= 200) {
      return '#fe0000';  // Red
  } else if (aqi <= 300) {
      return '#9a35ff';  // Purple
  } else {
      return '#930537';  // Maroon
  }
}
...
const renderDistrictData = (data) => {
  ...
  document.getElementById("best-district-aqi").style.color = getAQIColor(data.best_district.aqi.toFixed(0));
  
  $("#worst-district-aqi").text(data.worst_district.aqi.toFixed(0));
  document.getElementById("worst-district-aqi").style.color = getAQIColor(data.worst_district.aqi.toFixed(0));
